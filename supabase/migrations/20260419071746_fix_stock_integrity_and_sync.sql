/*
  # Fix stock integrity bugs and sync products.stock_quantity

  ## Changes
  1. Update post_stock_movement RPC to also sync products.stock_quantity = SUM(godown_stock)
     after every stock change. This ensures every page reading products.stock_quantity
     sees accurate totals, not just the Inventory page which re-computed it at load time.

  2. Fix cancel_delivery_challan: movement_type 'sale_return' → 'return' (constraint compliant).

  3. Expand stock_movements movement_type constraint to include 'sale_return' defensively.

  4. Add cancel_purchase_entry(p_entry_id uuid) RPC.

  5. One-time reconciliation: sync products.stock_quantity = SUM(godown_stock) for every product.
*/
